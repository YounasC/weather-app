...
import clearWeatherBg from "./assets/clear-sky-background-image.jpg";
import rainyWeatherBg from "./assets/rainy-background-image.jpg";
import snowWeatherBg from "./assets/snow-background-image.jpg";
import sunnyWeatherBg from "./assets/sunny-sky-background.jpg";
import cloudyWeatherBg from "./assets/cloudy-sky-background-image.jpg";
import thunderstormWeatherBg from "./assets/thunderstom-background-image.jpg";
...
function App() {
  ...
  const weatherBackground = {
    ...
    Clear: clearWeatherBg,
    Rain: rainyWeatherBg,
    Drizzle: rainyWeatherBg,
    Clouds: cloudyWeatherBg,
    Snow: snowWeatherBg,
    Sunny: sunnyWeatherBg,
    Thunderstorm: thunderstormWeatherBg,
  };
  ...
  const handleSearch = async (term) => {
    ...
    try {
      ...
      if (data && +data.cod === 200) {
        ...
        setBackgroundImage(
          weatherBackground[currentWeather] || weatherBackground.Default
        );
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 bg-black/40 text-white rounded-lg px-20 py-4 flex flex-col items-start space-y-2">
        <header className="flex items-center space-x-4 text-4xl font-semibold">
          ...
          <img
            ...
            alt="React Logo"
            className="react-logo w-12 h-12"
          />
        </header>
      ...
      <div className="flex flex-col items-center flex-grow">
        <div className="relative w-full max-w-md mt-20 search-container">
          ...
          <input
            ...
            onFocus={() => setShowDropdown(true)} // show dropdown
            onClick={() => setShowDropdown(true)}
          />

          {/* Search History Dropdown (mobile) */}
          {showDropdown && searchHistory.length > 0 && (
            <div className="absolute top-full left-0 w-full bg-white/90 rounded-b-lg shadow-md mt-1 overflow-hidden md:hidden transition-all duration-300">
        ...
        <div className="h-4 mt-1 flex items-center justify-center">
          {error && (
            <p className="text-sm text-red-500 bg-white/60 px-1 py-1 rounded">
              {error}
            </p>
          )}
        </div>
      ...
      <footer className="text-center py-8 bg-black/40 text-white mt-10">
        <p className="text-sm">
          @ {new Date().getFullYear()} My Weather App - Built using React &
          Tailwind
        </p>
      </footer>
    </div>
